      ...
      tickFn: ({node}) => {
        ...
        */
        ...
        async function fn (cell) {
          ...
          const color = chroma(colorFn({cell}))
          const rgba = color.rgba()
          rgba[3] = ~~(rgba[3] * 255)
          const rgbaForBlack = [0, 0, 0, 255]
          let rgbaForComplement = (() => {
            //const rotation = ~~(Math.random() * 360)
            const rotation = 120
            const c = color.set('hsl.h', (color.get('hsl.h') + rotation) % 360)
            const rgba_ = c.rgba()
            rgba_[3] = ~~(rgba_[3] * 255)
            return rgba_
          })()
          // rgbaForComplement = [0, 255, 0, 255]
          for (let y = 0; y < imageData.height; y++) {
            const rowStartIdx = y * imageData.width * 4
            for (let x = 0; x < imageData.width; x++) {
              const rgbaToUse = (x % 2) ? rgba : rgbaForComplement
              const pixelStartIdx = rowStartIdx + x * 4
              for (let i = 0; i < 4; i++) {
                imageData.data[pixelStartIdx + i] = rgbaToUse[i]
              }
            }
          }
